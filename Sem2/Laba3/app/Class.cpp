...
namespace zk {
    ...
    void clock::add_minutes(int added) {
        ...
        if (minutes_ >= 60) {
            ...
            if (hours_ >= 24) {
                int days_added = 0;
                if (battery.current_charge >= hours_ / 24) {
                    days_added = hours_ / 24;
                    battery.current_charge -= days_added;
                    day_ += days_added;
                }
                else {
                    while (battery.current_charge > days_added){
                        days_added += hours_ - 24;
                    }
                    battery.current_charge -= days_added;
                    day_ += days_added;
    ...
    void clock::print() const {
        std::cout << day_ << '.' << month_ << '.' << year_ << ' ' << hours_ << ':' << minutes_ << ' ' << day_of_week_ << ' ' << battery.current_charge << " [";
